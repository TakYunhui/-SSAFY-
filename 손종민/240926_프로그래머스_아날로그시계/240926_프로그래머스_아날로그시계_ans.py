def solution(h1, m1, s1, h2, m2, s2):
    ...
    def hour_check(hour, sec, next_hour, next_sec): # 시침이 초침과 만나는 경우
    ...
    def min_check(min, sec, next_min, next_sec): # 분침이 초침과 만나는 경우
    ...
    while elapsed_time <= time:
        ...
        if min_check(pre_min, pre_sec, next_min, next_sec):
            answer += 1
        if pre_sec == pre_hour or pre_min==pre_sec: # 예외처리 1
            answer += 1
        pre_hour = next_hour
        pre_min = next_min
        pre_sec = next_sec
        elapsed_time += 1
    if next_sec == next_hour or next_min ==next_sec: # 예외처리 2
        answer += 1
    return answer